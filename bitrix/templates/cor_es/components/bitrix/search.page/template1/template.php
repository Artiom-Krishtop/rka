...
<?elseif($arResult["ERROR_CODE"]!=0):?>
    ...
    <?$error = str_replace("Пустой поисковый запрос (запрос не содержит букв или цифр).","Пусты пошукавы запыт (запыт не ўтрымлівае літар або лічбаў).",$arResult["ERROR_TEXT"]);?>
	<?ShowError($error);?>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		<a href="<?echo $arItem["URL"]?>"><?echo str_replace('НЕ УДАЛЯТЬ','Сообщение форума',$arItem["TITLE_FORMATED"])?></a>
        <?php
        /*print "<pre>";
        print_r($arResult["DROPDOWN"]);
        print "</pre>";*/
        ?>
