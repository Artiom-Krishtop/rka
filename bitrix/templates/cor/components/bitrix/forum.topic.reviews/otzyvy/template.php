...
foreach ($arResult["MESSAGES"] as $res):
	...
	?><table cellspacing="0" border="0" class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		?>" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$res["LAST_NAME"]." ".$res["NAME"]." ".$res["SECOND_NAME"]?>" id="message<?=$res["ID"]?>">
		<thead><tr><td><?
		if ($arParams["SHOW_AVATAR"] != "N")
		{
			?><div class="review-avatar"><?
				if(is_array($res["AVATAR"]) && array_key_exists("HTML", $res["AVATAR"])): ?><?=str_replace('/upload/resize_cache/forum/avatar/8b5/30_30_2/ed44b0716b05c109ca588440d2d7be34.jpg','/bitrix/components/bitrix/forum.topic.reviews/templates/.default/images/noavatar.gif',$res["AVATAR"]["HTML"])?><?
...
<div>
		<b><?
		if (intval($res["AUTHOR_ID"]) > 0 && !empty($res["EMAIL"])):
            if ($res["LAST_NAME"]." ".$res["NAME"]." ".$res["SECOND_NAME"] == "Сидоров Иван Петрович"){
                echo $res["AUTHOR_NAME"];
            }else{
                echo $res["LAST_NAME"]." ".$res["NAME"]." ".$res["SECOND_NAME"];
            }
			/*?><a href="<?=$res["AUTHOR_URL"]?>"><??><?=$res["LAST_NAME"]." ".$res["NAME"]." ".$res["SECOND_NAME"]?><??></a><?*/
		else:
			?><?=$res["AUTHOR_NAME"]?><?
		endif;
		?></b>
	...
	<tbody>
	<tr><td>
		<div class="reviews-text" id="message_text_<?=$res["ID"]?>">
            <?if ($USER->IsAdmin()){?>
                IP-адресс: <?=$res["AUTHOR_IP"]?>
            <?}?>
