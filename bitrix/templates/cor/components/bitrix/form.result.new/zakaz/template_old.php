					...
					foreach ($arResult['QUESTIONS'] as $code => $arField) {
						...
						if ($arField['STRUCTURE'][0]['FIELD_TYPE']=='text') {
							...
							?><div class="col col-md-6 form-group field-wrap text-wrap <?if ($arField['REQUIRED']=="Y") {?>req<?}?> <?if (in_array($arField['STRUCTURE'][0]['QUESTION_ID'], $laststep)):?>laststep<?else:?>firststep<?endif;?>"><?
								...
								?></label><?
								?><input <?if ($arField['STRUCTURE'][0]['QUESTION_ID']=='3' or $arField['STRUCTURE'][0]['QUESTION_ID']=='8'):?>type="number" min="1"<?else:?>type="text"<?endif;?> name="form_text_<?=$arField['STRUCTURE'][0]['ID']?>" class="<?if ($arField['REQUIRED']=="Y") {?>req-input<?}?> form-control" value="<?=$arResult["arrVALUES"]["form_text_".$arField['STRUCTURE'][0]['ID']]?>"><?
							?></div><?
							if ($rowCounter%2!=0) {
								?></div><?
							}
						...
						/* INPUT TYPE ___RADIO___ */
							...
							?><div class="col col-md-6 field-wrap radio-group choice-wrap <?if ($arField['REQUIRED']=="Y") {?>req<?}?> <?if (in_array($arField['STRUCTURE'][0]['QUESTION_ID'], $laststep)):?>laststep<?else:?>firststep<?endif;?>"><?
								?><label class="control-label" for="<?=$code?>"><?
									?><?=$arField['CAPTION']?><?if ($arField['REQUIRED']=="Y") {?><span class="required"> <?=$arResult['REQUIRED_SIGN']?></span><?}?><?
								?></label><?
...
$('.firststep input[type=text], .firststep input[type=number], .firststep select').change(function() {
    var allfield = true;
    $('.firststep input[type=text], .firststep input[type=number], .firststep select').each(function(index, elem) {
       valu = $(elem).val();
       if (valu=='') {
