...
/*    $id_blog = str_replace('IBLOCK_','',$topic["XML_ID"]);
    ...
    if ($ob = $res->GetNextElement()){;
        ...
        while ($ar_res = $db_res->Fetch())
        {
            ...
            //$str = preg_replace('#(<et>(.*?)<\/et>)#is', '', $arMessage['POST_MESSAGE']);
            $str = preg_replace("/\[[^\]]*\]/", '', $arMessage['POST_MESSAGE']);
            //preg_match($arFields['DETAIL_TEXT'], $str, $match);
            //if(strstr($str,$arFields['DETAIL_TEXT']))
               // $str = '';
            //var_dump($match);
                /*echo "<pre><div>";
                print_r($arMessage);
			...
			<tbody>
				<tr class="forum-row-first forum-row-last forum-row-odd">
					<td class="forum-column-alone">
						<div class="forum-empty-message">Для Ваших ответов нет комментариев.<br />
<?/*
if ($arResult["USER"]["RIGHTS"]["CAN_ADD_TOPIC"] == "Y"):
?>
				...
				<tr class="<?=($iCount == 1 ? "forum-row-first " : "")?><?
					...
					<td class="forum-column-title">
						...
						if (false && strLen($res["IMAGE"]) > 0):
								...
								?><a href="<?=$res["LINKS"]?>#commento" target="_blank" title="<?=GetMessage("F_TOPIC_START")?> <?=$res["START_DATE"]?>"><?=$res["TITLE"]?></a><?
...
<?
						...
						if ($arResult["PERMISSION"] >= "M" && $res["mCnt"] > 0):
?>
					<td class="forum-column-replies forum-cell-hidden"><span ><?=$res["POSTS"]?> <?
						?>(<noindex><span style="color:#85092d;"><?=$res["mCnt"]?></span></noindex>)</span></td>
<?
						else:
?>
...
?>
						<div class="forum-lastpost-box">
							<span style="color:#85092d;" class="forum-lastpost-date"><noindex><?=$res["LAST_POST_DATE"]?></noindex></span>
