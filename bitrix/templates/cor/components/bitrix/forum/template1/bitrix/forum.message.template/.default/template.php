...
<?
		if ($res["AUTHOR_ID"] > 0):
            ...
            $arFilter = Array("IBLOCK_ID"=>17,"PROPERTY_USER"=>$res['AUTHOR_ID']);
            $reset = CIBlockElement::GetList(Array(),$arFilter, false, Array(), Array());
            if ($obres = $reset->GetNextElement()){
                $arFields = $obres->GetFields(); // поля элемента
                $ar_res["LINKS"]=$arFields["DETAIL_PAGE_URL"];
                $ar_res["NAME_ADVO"]=$arFields["NAME"];
                $ar_res["FOTO"]=$arFields["PREVIEW_PICTURE"];

            }
            $link = CFile::GetPath($ar_res['FOTO']);
            $src ="<img width='65%' src='".$link."'>";
?>
            <div class="forum-user-name">
                    <?if($res["AUTHOR_NAME"]=="test test "){?>
                    Гость
                    <?}else{?> 
<?=str_replace(array("#URL#", "#NAME#"), array($ar_res["LINKS"], $ar_res["NAME_ADVO"]), $arParams["USER_TMPL"])
					?>              
                     <?}?>                      
...
?>
					<div class="forum-user-avatar"><?
						?><?=str_replace(array("#URL#", "#NAME#"), array($ar_res["LINKS"], $src), $arParams["USER_TMPL"])
					?></div>
<?
			else:
?>
...
?>
					<div class="forum-user-name"><span>
                    <?if($res["AUTHOR_NAME"]=="test"){?>
                       Гость
                    <?}else{?> 
                    <?=$res["AUTHOR_NAME"]?>                    
                     <?}?>                    
...
//$reset = CUser::GetByID($res['AUTHOR_ID']);
//if($ar_res = $reset->GetNext())
/*print "<pre>";
print_r($ar_res);
print "</pre>";*/
	
				 /*$arFilter = Array("IBLOCK_ID"=>16,"PROPERTY_USER"=>$arResult["PROPERTIES"]["USER"]["VALUE"]);	
					$resew = CIBlockElement::GetList(Array('SORT' => 'ASC', 'ID' => 'DESC'),$arFilter, false, Array(), Array());				
					while ($obew = $resew->GetNextElement()){
						$arFields = $obew->GetFields(); // поля элемента
						$arProps = $obew->GetProperties();
...
<img src="/bitrix/components/bitrix/forum.topic.reviews/templates/.default/images/noavatar.gif" border="0" />
                    <?}
                    /*else{?>
<img width="20%" src="<?=CFile::GetPath($ar_res['PERSONAL_PHOTO'])?>" alt="<?=$ar_res['LOGIN']?>">
                    <?}*/?>
...
?>
			...
			<td class="forum-cell-post">
				...
				if ($arRes["USER"]["PERMISSION"] >= "Q" && $res["SHOW_CONTROL"] != "N"):
					...
					<?endif;?>
					<span><?=FormatDate('d F Y',MakeTimeStamp($res["POST_DATE"]), time())?><?//=$res["POST_DATE"]?></span>
				</div>
