...
<?
		if ($res["AUTHOR_ID"] > 0):
            ...
            $us_name = "<b>".$res["AUTHOR_NAME"]."</b>";
            $us_name2 = "<b>".$res["LAST_NAME"]." ".$res["NAME"]." ".$res["SECOND_NAME"]."</b>";

?>
					<div class="forum-user-name">
                        <?if(!empty($ar_res["LINKS"] && $ar_res["NAME_ADVO"]!="Сидоров Иван Петрович")){?>
                            <?=str_replace(array("#URL#", "#NAME#"), array($ar_res["LINKS"], $ar_res["NAME_ADVO"]), $arParams["USER_TMPL"])?>
                        <?}elseif($ar_res["NAME_ADVO"] == "Сидоров Иван Петрович"){?>
                            <?echo $us_name;
                            }else{
                            echo $us_name2;
                            //}?>
                            <b><?//=$res["AUTHOR_NAME"]//=str_replace(array("#URL#", "#NAME#"), array("", $res["AUTHOR_NAME"]), $arParams["USER_TMPL"])?></b>
                        <?}?>
                    </div>
<?
			if (is_array($res["AVATAR"]) && !empty($res["AVATAR"]["HTML"]) && $ar_res["NAME_ADVO"]!="Сидоров Иван Петрович"):
?>
