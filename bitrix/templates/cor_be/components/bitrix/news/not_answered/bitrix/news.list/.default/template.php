...
<div class="news-list view view-faq view-id-faq view-display-id-page">
    ...
    <div class="view-header" style="margin-bottom: 20px;">
        <?$count = count($arResult['ITEMS']);
          $count_all = count($arResult["NOT_NAV_COUNTB"]);
       // print "<pre>";
        //print_r($arResult["NOT_NAV_COUNTB"]);
        //print "</pre>";
        ?>
        <?if($count!=0){?>
        <h2>Вопросы с 1 до <?=$count;?> из <?=$count_all;?> неотвеченных вопросов </h2>
        <?}?>
    </div>
    ...
    <?foreach($arResult["ITEMS"] as $arItem):?>
        ...
        <div class="news-item views-row views-row-even" id="<?=$this->GetEditAreaId($arItem['ID']);?>" style="margin-bottom:40px;">
            <?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
                <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                    <?echo substr($arItem["~PREVIEW_TEXT"],-500)?>...<br />
                <?else:?>
        ...
        <div class="">
            ...
            <a href="index_ans.php?answer=<?=$arItem['ID']?>" >ответить</a> |
            <?
