						...
						/* INPUT TYPE ___EMAIL___ */
							...
							?><div class="col col-md-12 form-group field-wrap text-wrap <?if ($arField['REQUIRED']=="Y") {?>req<?}?>"><?
								?><label class="control-label" for="<?=$code?>"><?
									?><?=$arField['CAPTION']?><?if ($arField['REQUIRED']=="Y") {?><span class="required"> <?=$arResult['REQUIRED_SIGN']?></span><?}?><?
								?></label><?
								?><input type="text" value="<?=$arResult['arrVALUES']['form_email_'.$arField['STRUCTURE'][0]['ID']]?>" name="form_email_<?=$arField['STRUCTURE'][0]['ID']?>" class="<?if ($arField['REQUIRED']=="Y") {?>req-input<?}?> form-control"><?
							?></div><?
							if ($rowCounter%2!=0) {
								?></div><?
							}
