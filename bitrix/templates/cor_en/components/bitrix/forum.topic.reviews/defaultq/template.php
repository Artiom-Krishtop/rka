		...
		<? }else{?>
			...
			<? }else{?>
			   ...
			   <?=$res["NAME"]?>
				...
				<?}?>				
			<?}?>
			<?//=$rsUsers["arResult"][0]["NAME"]."<br />".$rsUsers["arResult"][0]["SECOND_NAME"]."<br />".$rsUsers["arResult"][0]["LAST_NAME"]?>			
			<?
		 }	
		endif;
		?></b>
<? if ($res["AUTHOR_ID"] == 3 && $res["ID"] == 25){	?>
		<span class='message-post-date'>2.03.2018 16:55:44</span>
<? }else{?>
...
<? }?>		
	...
	<tbody>
	<tr><td>
		<noindex><div class="reviews-text" id="message_text_<?=$res["ID"]?>"><?=$res["~POST_MESSAGE_TEXT"]?></div></noindex>
<?
