...
if($_SERVER["REQUEST_METHOD"]=="POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && check_bitrix_sessid())
{
	...
	if($strError == '')
	{
		if (CModule::IncludeModule("forum"))
		{
            ...
            if(isset($_REQUEST["forum_ALLOW_POST"]))
            {
                $arforumFields["ALLOW_POST"] = ($_REQUEST["forum_ALLOW_POST"] == "Y"? "Y" : "N");
            }
            if(isset($_REQUEST["forum_HIDE_FROM_ONLINE"]))
            {
                $arforumFields["HIDE_FROM_ONLINE"] = ($_REQUEST["forum_HIDE_FROM_ONLINE"] == "Y"? "Y" : "N");
            }
            if(isset($_REQUEST["forum_SUBSC_GET_MY_MESSAGE"]))
            {
                $arforumFields["SUBSC_GET_MY_MESSAGE"] = ($_REQUEST["forum_SUBSC_GET_MY_MESSAGE"] == "Y"? "Y" : "N");
            }
